...
metro_trips <- tibble::tribble(
  ...
  "M1_002", "3", "M1_002_N_3_M1_001", "M1_001")
```
...
```{r}
raw_metro_line$shape_id = "M1_001"
```
...
```{r}
metro_shapes <- convert_sf_to_shapes(raw_metro_line, shape_id = NULL)
```
...
```{r}
metro_stop_times_N$arrival_time <- as_hms("04:00:00")
metro_stop_times_N$departure_time <- as_hms("04:00:00")
...
```{r}
cum_cutoff_29_brt <- cumulative_cutoff(
  ttm_brt,
  land_use_data = hex_points_main,
  opportunity = "LU_COD_21",
  travel_cost = "travel_time",
...
Merging r5r results with hexagon grid (not in use)

```{r}
# spatial_r5r_brt <- merge(
#   hex_grid_9,
#   r5r_access_brt,
#   by.x = "h3_index",
#   by.y = "id"
...
ggplot(main_results_hex9) +
  ggtitle("BRT accessibility: cutoffs = 30  max_walk_time = 30, max_trip_duration = 120") + 
...
```{r}
cum_cutoff_29_metro <- cumulative_cutoff(
  ttm_metro,
  land_use_data = hex_points_main,
  opportunity = "LU_COD_21",
  travel_cost = "travel_time",
...
```{r}
cum_cutoff_29_full <- cumulative_cutoff(
  ttm_full,
  land_use_data = hex_points_main,
  opportunity = "LU_COD_21",
  travel_cost = "travel_time",
...
```{r}
ggplot(main_results_hex9) +
  ggtitle("BRT and Metro accessibility: cutoffs = 30  max_walk_time = 30, max_trip_duration = 120") + 
...
```
__Maximum Walking time extremely low__
...
##### Maximum travel time (change to use median in filtering)
...
```{r}
ttm_walk_max<- main_ttm_hex9 %>%
  filter(walk_tt == max(walk_tt))
  #list_ttm_walk_max <- list(main_ttm_hex9$from_id)
```

List

```{r}
list_ttm_walk_max <- as.list(ttm_walk_max$from_id)
```

Subsetting main reuslts

```{r}
main_results_hex9[main_results_hex9$h3_index %in% list_ttm_walk_max, ]
```


```{r}
ggplot(main_results_hex9[main_results_hex9$h3_index %in% list_ttm_walk_max, ]) +
  ggtitle("Hexagons with max ttm in walking") + 
  geom_sf(aes(fill = a_brt_metro_60), color = NA) +
  scale_fill_viridis_c() +
  labs(fill = "Accessible\nHigh Street Retail") +
  #theme_minimal()
  theme(plot.title = element_text(size = 8, face = "bold"))
...
```{r}
list_ttm_walk_min <- as.list(ttm_walk_min$from_id)
```

Subsetting main reuslts

```{r}
main_results_hex9[main_results_hex9$h3_index %in% list_ttm_walk_min, ]
```


```{r}
ggplot(main_results_hex9[main_results_hex9$h3_index %in% list_ttm_walk_min, ]) +
  ggtitle("Hexagons with max ttm in walking") + 
  geom_sf(aes(fill = a_brt_metro_60), color = NA) +
  scale_fill_viridis_c() +
  labs(fill = "Accessible\nHigh Street Retail") +
...
```{r}
main_results_hex9<- rename(main_results_hex9, b_brt_60 = accessibility)
```
...
```{r}
ggplot(main_results_hex9) +
  ggtitle("BRT accessibility: cutoffs = 60  max_walk_time = 30, max_trip_duration = 60") + 
  geom_sf(aes(fill = b_brt_60), color = NA) +
  scale_fill_viridis_c() +
  labs(fill = "Accessible\nHigh Street Retail") +
  #theme_minimal()
  theme(plot.title = element_text(size = 8, face = "bold"))
...
```{r}
main_results_hex9<- rename(main_results_hex9, m_metro_60 = accessibility)
```
...
```{r}
ggplot(main_results_hex9) +
  ggtitle("Metro accessibility: cutoffs = 60  max_walk_time = 30, max_trip_duration = 60") + 
  geom_sf(aes(fill = m_metro_60), color = NA) +
  scale_fill_viridis_c() +
  labs(fill = "Accessible\nHigh Street Retail") +
  #theme_minimal()
  theme(plot.title = element_text(size = 8, face = "bold"))
...
```{r}
main_results_hex9<- rename(main_results_hex9, a_brt_metro_60 = accessibility)
```
...
```{r}
ggplot(main_results_hex9) +
  ggtitle("BRT and Metro accessibility: cutoffs = 60  max_walk_time = 30, max_trip_duration = 60") + 
  geom_sf(aes(fill = a_brt_metro_60), color = NA) +
  scale_fill_viridis_c() +
  labs(fill = "Accessible\nHigh Street Retail") +
  #theme_minimal()
