...
```{r}
geo_metro_stations <- st_read("Data/Raw/Transport/Metro/Stations/ESTACIONES.shp") %>%
  st_transform(.,4326)
```
Review CRS

```{r}
st_crs(geo_metro_stations)
```
Geometry

```{r}
qtm(geo_metro_stations)
```
...
```{r}
geo_metro_line <- st_read("Data/Raw/Transport/Metro/Track/TRAZADO_PLMB.shp") %>%
  st_transform(.,4326)
```
...
```{r}
geo_metro_line$shape_id = "M1_001"
```
...
```{r}
metro_line_shape <- convert_sf_to_shapes(geo_metro_line, shape_id = NULL)
```
