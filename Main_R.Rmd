...
Merging results with main result matrix
...
## Hexagon measure compilation (to be moved to every section)
...
```{r}
median(spatial_r5r_brt$accessibility)
```
Max duration: 60 min

```{r}
median(spatial_r5r_brt_1$accessibility)
```
### Metro
...
```{r}
median(spatial_r5r_metro$accessibility)
```
Cuttoffs: 60 min

```{r}
median(spatial_r5r_metro_1$accessibility)
```
...
```{r}
median(spatial_r5r_full$accessibility)
```
Cuttoffs: 60 min

```{r}
median(spatial_r5r_full_1$accessibility)
```
