...
```{r}
local_path <- "Data/Transport/BRT/GTFS-2023-06-21.zip"
```
...
```{r}
osm_network <- oe_read("Data/Transport/OSM/Bogota_general_network.osm.pbf") 
```
...
```{r}
geo_zpu <- st_read("Data/Demo/Survey/Shape/ShapeEM2021/EM2021_UPZ.shp")
```
...
```{r}
survey_raw <- read_csv("Data/Demo/Survey/CSV_Main/20230620EM2021.csv")
```
...
```{r}
survey_add_raw <- read_csv("Data/Demo/Survey/CSV_Additional/20230220_variables_adicionales_em2021.csv")
```
...
```{r}
land_use_terrain <- st_read("Data/Land_use/Terrain/DestinoSHP/Destino.shp")
```
Geometry
...
```{r}
# BRT GTFS data
file.copy(from = "Data/Transport/BRT/GTFS-2023-06-21.zip", to = file.path(before_dir, "brt_gtfs.zip"))

# OSM Street network
file.copy(from = "Data/Transport/OSM/Bogota_general_network.osm.pbf", to = file.path(before_dir, "street_network.osm.pbf"))
