...
survey_add_zpu <- survey_add_homes %>%
  group_by(cod_upz_grupo) %>%
  summarize(count_homes = n(),income_mean_mean_zpu = mean(income_mean,na.rm = TRUE), income_median_median_zpu = median(income_median,na.rm = TRUE), income_median_mean_zpu = mean(income_median,na.rm = TRUE))
```
...
ggplot() +
  ...
  theme_minimal() +
  scale_color_manual(values = legend_data$color, labels = legend_data$label) + labs(color=NULL)
```
...
ggsave("Demo_income_decile.png", path = "Data/Results/Images/")
```
#### Scarerplot
...
ggplot(main_results_hex9) +
  ...
  labs(
    x = "Income decile",
    y = "High Street Retail Opportunities\n(thousands)") +
...
ggplot(main_results_hex9_pivot_longer) +
  ...
  facet_wrap(
    ~ scenario,
    nrow = 1,
    labeller = as_labeller(c(b_brt_30 = "Before", a_brt_metro_30 = "After"))
  ) +
  scale_colour_viridis(discrete = TRUE) +
  labs(x = "Income decile", y = "High Street Retail Opportunities (thousands)") +
  scale_x_discrete(
    labels = c("D1\npoorest", paste0("D", 2:9), "D10\nwealthiest")
  ) #+ theme_minimal()
...
ggplot(main_results_hex9) +
  ...
  labs(
    x = "Income decile",
    y = "High Street Retail Opportunities difference\n(thousands)") +
  scale_x_discrete(labels = c("D1\npoorest", paste0("D", 2:9), "D10\nwealthiest")) #+ theme_minimal()
