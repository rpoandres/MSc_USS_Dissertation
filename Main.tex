...
\keywords{Accessibility modelling - transport - land use - GTFS - Bogotá - Metro - sustainable transport}
...
\thispagestyle{empty}
\chapter*{Declaration}
I, Andr\'{e}s Restrepo Jim\'{e}nez, hereby declare that this dissertation is all my own original work and that all sources have been acknowledged. It is 10.658 words in length. \\
...
\begin{enumerate}
  ...
  \begin{itemize}
      ...
      \item \cite{battyNewFrameworkVery2021} developed a model called QUANT based on land use–transport interaction (LUTI) to simulate the effects on population and employment when possible land use policies variation and transport infrastructure projects are conducted in the United Kingdom.
  ...
  \end{itemize}
  \item Urban dynamics models: Built on the principles used in ecological and system dynamics models like "predator and pray models" \citep{battyUrbanModeling2009a}, the urban dynamics models intend to derive the aggregate future dynamic activity or state that may result within an urban system through time.
  \begin{itemize}
      \item \cite{wangSystemDynamicsModel2008} used an urban system dynamics model to predict the state and feedback of several subsystems that represented built environment variables of interest in the Chinese city of Dalian. The subsystems were related to "population, economic development, number of vehicles, environmental influence, travel demand, transport supply, and traffic congestion" \citep{wangSystemDynamicsModel2008}.
      \item \cite{qiSystemDynamicsModeling2011} used a system dynamic model to predict the water demand based on the "...intrinsic relationship between water demand and macroeconomic environment..." \citep{qiSystemDynamicsModeling2011}.
  \end{itemize}
  ...
  \begin{itemize}
      \item   \cite{battyCellularAutomataUrban1997} demonstrated how CA could illustrate spatial organization in cities through a decentralized simulation decision-making process.
  ...
  \begin{itemize}
      \item \cite{arcauteCitiesRegionsBritain2016} explored the urban and regional spatial hierarchical structures in the United Kingdom by applying different distance thresholds (percolation theory) in the street network.
      \item \cite{zhongDetectingDynamicsUrban2014} conducted network analysis to examine the spatial structure of Singapore using daily "smart card" transit data.
  \end{itemize}
\end{enumerate}
...
Given these points, it was highlighted by \cite{battyUrbanModeling2009a} and \cite{wilsonFutureUrbanModelling2018} how these modelling tools hold great value for both policymakers and private parties, as they manage to integrate theoretical constructs or business strategies and put them to the test through computational modelling \citep{battyUrbanModeling2009a}. From a public, private or research perspective, modelling tools applied to the urban context could provide decision-makers with new innovative insights and possible solutions to the numerous challenges that cities and society currently face across the globe. Furthermore, \cite{battyUrbanModeling2009a} clarified that apart from the computational models mentioned above, GIS-based models are also part of the urban modelling domain and will be described in the next section of the present document.

As can be seen, the previous exercises together aimed to predict a future or alternative scenario by conducting urban modelling simulation. Urban modelling can provide valuable insight into the complex and interrelated phenomena that are the urban environment.

% Used simulation to model future urban scenarios and deppe understanding on intertwined relations that happened in cities and regions
...
In that sense, GIS can be used as infrastructure and platform to address a wide range of location-sensitive topics as they enable and support spatial modelling processes to test future scenarios according to the applicable use case. Combining urban modelling techniques and GIS-based analysis enables many opportunities when spatially assessing a future or alternative scenario in the built environment. 
...
The main common purpose of previous research conducted in Bogotá was to assess the accessibility impact of the transportation system's future expansion, integration or fare scheme introduction. The accessibility measures were mostly assessed from a social and access-inequality perspective as results were disaggregated by population groups of interest. Overall, the conducted study falls in the same direction explored in the previous experiences regarding using accessibility measures to assess the suitability of a transport-related decision. Among the accessibility-oriented assessment, the novel feature of this study is the simulation of the future commuting conditions that the metro system would offer, together with the BRT's current operation.
