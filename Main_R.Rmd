...
**It seems that there are some trips with no geometries.**
...
**How to interpret the stop_sequence field?**
...
**Do a group by with 0 and 1 values in stop sequence**
...
**Take these trips and drops them from the sf object** **Review if these are really inconsistent**
...
**Is this adjusted when using the street network?**
...
```{r}
geo_from_stops_1x1_5 <- gtfstools::get_trip_geometry(brt_gtfs, trip_id = "BC26E0001-2-3")
```
...
```{r}
ggplot() +
  geom_sf(data = geo_from_stops_1x1_5) +
  theme_minimal()
```
...
```{r}
filter(brt_gtfs$stops, stop_id %in% c("8961","7178", "88574", "88130"))
```
...
\_Data downloaded in the wrong format\_\_
...
__What sub-setting should be done before the accessibility modelling?__
...
__Should be calculated for the H3 hegagon grid__
