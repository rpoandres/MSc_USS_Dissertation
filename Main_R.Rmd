...
Calculates the centrids of every station polygon
...
```{r}
metro_stop_times <- metro_stops %>%
  mutate(trip_id = metro_trips$trip_id[2]) %>%
  relocate(trip_id)
```

Temprary list with zero in the final value

```{r}
temp_time_to_next <- list()
temp_time_to_next <- as.list(distance_stations_df$dura_min)
temp_time_to_next <- append(temp_time_to_next,0)
```
...
metro_stop_times <- metro_stop_times %>%
  mutate(time_to_next = unlist(temp_time_to_next))
...
```{r}
metro_stop_times$arrival_time <- as_hms("04:00:00")
#metro_stop_times$arrival_time <- hms(metro_stop_times$arrival_time)
metro_stop_times$departure_time <- as_hms("04:00:00")
#metro_stop_times$departure_time <- hms(metro_stop_times$departure_time)
...
for (i in 2:(nrow(metro_stop_times) - 1)) {
  
  #metro_stop_times$arrival_time[i] = as_hms(metro_stop_times$departure_time[i-1]+ as_hms("00:00:55"))
  #metro_stop_times$arrival_time[i] = as_hms(metro_stop_times$departure_time[i-1]+ as_hms(paste("00:00:",(metro_stop_times$time_to_next[i-1]*60))))
  
  #metro_stop_times$arrival_time[i] =  as_hms(time_add(as.POSIXct(metro_stop_times$departure_time[i-1]), minutes = 55))
  
  #as_hms(time_add(as.POSIXct(time), minutes = 55))
  
  metro_stop_times$arrival_time[i] = as_hms(metro_stop_times$departure_time[i-1] + hms(seconds = ceiling(metro_stop_times$time_to_next[i-1]*60)))
  
  
  
  metro_stop_times$departure_time[i] = metro_stop_times$arrival_time[i]
...
__Setting initial arrival time value__



__Add duration time to previous departure time__
